      ...
      <div className={Styles.box}>
        <form action="a">
          <div className={Styles.input_box}>
            ...
            <div className={Styles.selection} onClick={handleSelectionClick}>Selection</div>
            <div className={`${Styles.categories} ${isActive ? Styles.active : ''}`}>
      ...
      <div className={Styles.items_container}>
        {items.map((item) => (
          <div key={item.id} className={Styles.item}>
            ...
            <div className={Styles.type} style={{width: '70px', background: '#141b7b'}}>{item.type}</div>
            <div className={Styles.status}  style={{width: '70px', background: 'green'}} >{item.status}</div>
            <div className={Styles.date}  style={{width: '100px', background: '#736766'}}>{item.date}</div>
          </div>
        ))}
      </div>
      </div>
